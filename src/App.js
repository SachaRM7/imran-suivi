...
const CAT_COLORS = {
  ...
  "Protéines": "#A855F7", "Laitiers": "#3B82F6"
};
...
const TeethSection = ({ data, update }) => {
  ...
  const top = TEETH_MAP.filter(t => t.pos === "top");
  const bot = TEETH_MAP.filter(t => t.pos === "bottom");
  ...
  return (
    <div>
      ...
      <Card style={{ padding: 20, marginBottom: 16, textAlign: "center" }}>
        <div style={{ fontSize: 12, fontWeight: 700, color: "#6B7280", marginBottom: 10 }}>Mâchoire supérieure</div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, justifyContent: "center", marginBottom: 18 }}>
          {top.map(t => (
            <div key={t.id} onClick={() => toggle(t.id)} style={{ width: 38, height: 38, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: 18, background: data.teeth?.[t.id] ? "#FEF3C7" : "#F9FAFB", border: `2px solid ${data.teeth?.[t.id] ? "#FBBF24" : "#E5E7EB"}`, transition: "all .2s" }} title={t.name}>
              {data.teeth?.[t.id] ? "🦷" : "○"}
            </div>
          ))}
        </div>
        <div style={{ fontSize: 12, fontWeight: 700, color: "#6B7280", marginBottom: 10 }}>Mâchoire inférieure</div>
