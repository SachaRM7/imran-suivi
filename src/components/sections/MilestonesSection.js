...
const MilestonesSection = ({data,update,profile}) => {
  ...
  const toggle=(m,i)=>{
    ...
    if(!wasChecked){
      ...
      if(nowChecked===list.length){
        confetti({particleCount:150,spread:70,origin:{y:.6},zIndex:9999,disableForReducedMotion:true});
      }
    }
  };
