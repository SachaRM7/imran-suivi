...
const BathsSection = ({data,update}) => {
  ...
  return (
    <div>
      ...
      <Btn variant="secondary" full onClick={()=>update(d=>{d.baths.push({id:uid(),time:nowStr(),temp:37,note:""})})} style={{marginBottom:16,background:t.accentLight,border:`1.5px solid ${t.chipBorder}`,color:t.accent,fontWeight:700}}>🛁 Bain maintenant (37°C)</Btn>
      ...
      <Modal open={modal} onClose={()=>setModal(false)} title="Bain"><Input label="Heure" type="datetime-local" value={time} onChange={e=>setTime(e.target.value)}/><Input label="Temp eau (°C)" type="number" inputMode="decimal" pattern="[0-9]*" step=".5" value={temp} onChange={e=>setTemp(e.target.value)}/><Input label="Note" value={note} onChange={e=>setNote(e.target.value)}/><Btn onClick={add} full>Enregistrer</Btn></Modal>
    </div>
  );
};
